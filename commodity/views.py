...
def collectView(request):
    ...
    # 代表当前商品的主键id
    id=request.GET.get('id','')# 然后设置响应内容result，以字典的形式表示
    result = {"result":"已收藏"} # 最后从请求对象request获取会话Session数据likes，
    likes = request.session.get('likes',[]) # 如果存在数据，那么赋值给变量likes，否则变量likes设置为空列表
    # #setting中间件设置和install内置
    ...
    if id  and not int(id) in likes:
        ...
        # 实现字段likes的自增一操作
        CommodityInfos.objects.filter(id=id).update(likes=F('likes')+1)  # 然后将响应内容result改为收藏成功
...
#         )
        ...
        result["result"] = "收藏成功"   # 最后，将当前商品的id写入会话session数据likes，标记当前商品已经被用户收藏了
